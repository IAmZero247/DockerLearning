...
### Use case : Spring Boot with MySQL using Docker Compose
...
```
	...
	$ docker build -tag spring-boot-mysql-app ruby4eyes/spring-boot-mysql-app
...
8. ##### Run containers using docker compose ( docker-compose.yml available )
`$ docker-compose up -d`
